...
class DiscordClient(commands.Bot):
    ...
    @staticmethod
    def UpdateLastDropTime(groupName, botBatchIdInGroup, botIdInBatch, botId):
        DiscordClient.lastDropTime[groupName][botBatchIdInGroup][botIdInBatch] = datetime.datetime.utcnow()
        mongodbreaderwriter.UpdateDropTime(botId, DiscordClient.lastDropTime[groupName][botBatchIdInGroup][botIdInBatch])
    
    @staticmethod
    def UpdateLastPickupTime(groupName, botBatchIdInGroup, botIdInBatch, botId):
        DiscordClient.lastPickupTime[groupName][botBatchIdInGroup][botIdInBatch] = datetime.datetime.utcnow()
        mongodbreaderwriter.UpdatePickupTime(botId, DiscordClient.lastDropTime[groupName][botBatchIdInGroup][botIdInBatch])
    ...
    def UpdateCanDrop(groupName, botBatchIdInGroup, botIdInBatch):
        DiscordClient.canDrop[groupName][botBatchIdInGroup][botIdInBatch] = (int((datetime.datetime.utcnow() - DiscordClient.lastDropTime[groupName][botBatchIdInGroup][botIdInBatch]).total_seconds()//60) >= 32)
        ...
        for t in range(len(DiscordClient.canDrop[groupName][botBatchIdInGroup])):
            if DiscordClient.canDrop[groupName][botBatchIdInGroup][t]:
                if min_time > DiscordClient.lastDropTime[groupName][botBatchIdInGroup][t]:
                    DiscordClient.turnToDrop[groupName][botBatchIdInGroup] = t
    ...
    @staticmethod
    def CanDrop(groupName, botBatchIdInGroup, botIdInBatch):
        return DiscordClient.canDrop[groupName][botBatchIdInGroup][botIdInBatch]
    ...
    async def on_ready(self):
        ...
        #Card Drop Code
        while 1:   
            if (DiscordClient.CanDrop(self.botGroupName, self.botBatchIdInGroup, self.botIdInBatch) and DiscordClient.IsTurnToDrop(self.botGroupName, self.botBatchIdInGroup, self.botIdInBatch)) and DiscordClient.CanGroupPickup(self.botGroupName, self.botBatchIdInGroup):
                channel = self.get_channel(self.channelIdActiveIn)
                await channel.send("kd")
                DiscordClient.UpdateLastDropTime(self.botGroupName, self.botBatchIdInGroup, self.botIdInBatch, self.user.id)
    ...
    async def on_reaction_add(self, reaction, user):
        if (reaction.emoji == self.reactionToBeAdded and user.id == 646937666251915264) and (reaction.message.channel.id == self.channelIdActiveIn and DiscordClient.canPickup[self.botGroupName][self.botBatchIdInGroup][self.botIdInBatch]):
            for user in reaction.message.mentions:
                if user.id in DiscordClient.batches[self.botGroupName][self.botBatchIdInGroup]:        
                    await asyncio.sleep(random.random()+random.randint(1,3))
                    await reaction.message.add_reaction(self.reactionToBeAdded)
                    DiscordClient.UpdateLastPickupTime(self.botGroupName, self.botBatchIdInGroup, self.botIdInBatch, self.user.id)
                    DiscordClient.UpdateCanPickup(self.botGroupName, self.botBatchIdInGroup, self.botIdInBatch)